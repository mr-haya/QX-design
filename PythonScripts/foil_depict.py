...
def find_intersection_and_angle(curve, circle_center, radius):
    ...
    # 曲線の各座標間で、円との交点が存在するかを確認
    for i in range(len(curve) - 1):
        x1, y1 = curve[i]
        x2, y2 = curve[i + 1]
        
        # セグメントの両端の点が円の内外にあるか確認
        d1 = (x1 - h)**2 + (y1 - k)**2
        d2 = (x2 - h)**2 + (y2 - k)**2
        
        if (d1 <= radius**2 and d2 >= radius**2) or (d1 >= radius**2 and d2 <= radius**2):
            # 交点を求める（線形補間を使用）
            t = (radius**2 - d1) / (d2 - d1 + 1e-10)
            x_intersection = x1 + t * (x2 - x1)
            y_intersection = y1 + t * (y2 - y1)
            
            # 交点からの角度を計算
            angle = np.atan2(y_intersection - k, x_intersection - h)
