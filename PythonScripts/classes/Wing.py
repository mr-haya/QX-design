...
class Wing:
    def __init__(self):
        ...
        self.sht_wing = self.wb.sheets[sn.wing]
        # スパー生成
        self.spar = WingSpar()
        # 値読み込み
        self.b = self.sht_wing.range(ca.planform_cell).value[7][0] * 2
        self.weight = self.sht_wing.range(ca.weight_cell).value * 2  # 主翼重量
        self.planform = self.sht_wing.range(ca.planform_cell).value
