...
def show_data(type="wireframe",*args):
    ...
    for arg in args:
        ax.plot_wireframe(arg[0], arg[1], arg[2]) if type == "wireframe" else ax.plot_surface(arg[0], arg[1], arg[2]) if type == "surface" else ax.plot(arg[0], arg[1], arg[2])
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
...
def get_onedrive_path():
    if get_os_type() == 'windows':
        ...
        return {
            'commercial': "/Users/morihayato/OneDrive - Kyushu University",
            'consumer': "/Users/morihayato/OneDrive",
        }
...
def onedrive_url_to_local_path(url):
    ...
    if OneDriveCommercialUrlPattern in url:
        ...
        return os.path.join(onedrive_path['commercial'], url[FilePathPos:].replace("/", os.path.sep))
    else:
        ...
        if FilePathPos == -1:
            ...
            return os.path.join(onedrive_path['consumer'], url[FilePathPos:].replace("/", os.path.sep))

#%%
def get_file_path():
    wb = xw.Book.caller()
    script_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(script_dir, "..", const.BOOK_NAME)
