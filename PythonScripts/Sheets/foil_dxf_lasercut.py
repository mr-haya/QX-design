...
def main():
    ...
    for id in range(69 + 1):  # リブ番号の範囲を指定
        ...
        if is_half:
            ...
            offset_base = 0.45
        # リブオフセット
        dat_out = geo.offset_foil(offset_arr, offset_base)
        # リブ描写
        ...
        file_name = (
            "C:/Users/soyha/OneDrive - Kyushu University/AircraftDesign/QX-design/Outputs/master/rib_master_231020/"
            + str(id)
            + ".dxf"
        )
...
def add_square(msp, center, width, height, alpha=0):
    ...
    # Calculate half-diagonals for the rectangle
    dx = width / 2
    dy = height / 2
    ...
    # Add the rectangle to msp using the rotated corners (implementation depends on msp's API)
    for i in range(4):
        start_point = rotated_corners[i]
        end_point = rotated_corners[(i + 1) % 4]
        # Assuming msp has an add_line method similar to ezdxf's msp
